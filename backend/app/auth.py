...
SECRET_KEY = os.getenv("JWT_SECRET", "auditweave_super_secret_key_123456_secure_key")
ALGORITHM = "HS256"
...
async def get_current_user_optional(token: Optional[str] = Depends(oauth2_scheme_optional), db: Session = Depends(get_db)) -> User:
    """Optional auth dependency: parses JWT token if present, or falls back to a default guest auditor user."""
    if token:
        try:
            payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
            email: str = payload.get("sub")
